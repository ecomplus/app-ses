    ...
    try {
      ...
      if (order) {
        if (!customer) {
          ...
          postMetafields(appSdk, storeId, order._id, 'notification_error', 'Error with customer data, transactional mail not sent it.').catch(e => console.error(e))
        }
      }
    }
            ...
            if (mailIsEnable) {
              ...
              if (enableTo.customer) {
                ...
                sendMail(html, to, from, cc, store.name, subject)
                  .then((info) => {
                    if (notified) {
                      logger.log(`[debug]: ${subject} | ${mailTrigger} | ${JSON.stringify(notified)}`)
