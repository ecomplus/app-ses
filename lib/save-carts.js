    ...
    appSdk
      ...
      .then(data => {
        ...
        if (available && completed === false) {
          ...
          if (completed === true && data.customers) {
            data.customers.forEach((customerId) => {
              db.run('DELETE FROM ecomplus_cart_watch WHERE customers_ids = ?', [JSON.stringify([customerId])], err => {
                if (!err) {
                  logger.log(`> all carts of customer ${customerId} deleted`)
                }
              })
            })
          }
      ...
      .catch(err => {
        logger.error(`Error save cart ${cartId}/ #${storeId} => `, err)
      })
  }
}
