    ...
    try {
      ...
      carts.forEach(async (cartSQL) => {
        ...
        try {
          ...
          let createdAt = new Date(cartSQL.created_at).getTime()
          let finalDate = new Date().getTime()
          createdAt = createdAt + (configObj.is_abandoned_after_days * 24 * 60 * 60 * 1000)

          if (configObj.is_abandoned_after_days && finalDate >= createdAt) {
