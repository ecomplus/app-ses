    ...
    try {
      ...
      carts.forEach(async (cartSQL) => {
        ...
        try {
          ...
          const sendToTest = storeId === 1173

          if (finalDate === configObj.is_abandoned_after_days || sendToTest) {
            ...
            await appSdk.apiRequest(storeId, url)
              ...
              .catch(err => {
                if ((err.response && err.response.status === 404) || err.appWithoutAuth) {
          ...
          logger.error(`> Cart #${cartSQL.cart_id} Store #${storeId} error =>`, e)
        }
      })
