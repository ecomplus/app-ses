...
module.exports = (trigger, store, customer, order, lang) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    logger.log('new_order', !order.payments_history.find(entry => entry.customer_notified))
      ...
      switch (subresource) {
        case 'payments_history':
          if (!order.payments_history.find(entry => entry.customer_notified)) {
            ...
            logger.log('lastNotifiedStatus', lastNotifiedStatus)
            const lastCustomerNotified = order.payments_history.find(entry => entry._id === trigger.inserted_id && entry.customer_notified === false)
            logger.log('lastCustomerNotified', lastCustomerNotified)
            logger.log('valid', (lastNotifiedStatus !== body.status && lastCustomerNotified))
