...
module.exports = (trigger, store, customer, order, lang) => {
  return new Promise((resolve, reject) => {
    ...
    const { subresource, body } = trigger
    ...
    switch (subresource) {
      case 'payments_history':
        if (body.customer_notified === false && !order.payments_history.find(entry => entry.customer_notified)) {
          // new order
        ...
        } else if (body.customer_notified === false && order.payments_history.find(entry => (entry._id === trigger.inserted_id && entry.customer_notified === false))) {
