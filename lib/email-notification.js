...
module.exports = ({ appSdk, configObj }) => {
  return (trigger, storeId) => {
    ...
    let isRetry = false
    // set once the e-mail has actually been sent, so a retry only re-attempts
    // the "customer_notified" flag and never sends the same e-mail twice
    let notifiedRecordId = null
    ...
    const queueAndSend = () => {
      ...
      setTimeout(async () => {
        try {
          if (notifiedRecordId) {
            // e-mail already sent on a previous attempt, only the notified flag failed to save
            const url = `orders/${orderId}/${subresource}/${notifiedRecordId}.json`
            await appSdk.apiRequest(storeId, url, 'PATCH', { customer_notified: true })
              .catch(scheduleRetry)
            return
          }
          ...
          await Promise
            ...
            .then(({ info, options }) => {
              ...
              notifiedRecordId = statusRecordId
