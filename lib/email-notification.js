          ...
          .then(async ({ customer, order, store }) => {
            ...
            if (Array.isArray(order[subresource])) {
              ...
              if (!isCustomerNotified) {
                const sentNotifications = order[subresource]
                  .filter(entry => entry.customer_notified)
                  .sort((a, b) => a.date_time > b.date_time ? -1 : 1)
                if (sentNotifications.length) {
                  lastNotifiedStatus = sentNotifications[0].status
                }
              }
            }

            if (!isCustomerNotified && lastNotifiedStatus !== body.status) {
