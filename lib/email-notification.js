...
module.exports = ({ appSdk, configObj }) => {
  return (trigger, storeId) => {
    ...
    if (Number(storeId) === 1011) {
      console.log('Vamos comecar a processar o envio', JSON.stringify(body))
    }
    ...
    const queueAndSend = () => {
      ...
      if (Number(storeId) === 1011) {
        console.log('Processar fila', JSON.stringify(processQueue))
      }
      setTimeout(async () => {
        let lastValidRecord
        Promise
          ...
          .then(([store, order]) => {
            if (Number(storeId) === 1011) {
              console.log('Pegou loja', JSON.stringify(store), 'Pegou pedido', JSON.stringify(order))
            }
          ...
          .then(async ({ customer, order, store }) => {
            if (Number(storeId) === 1011) {
              console.log('Cliente buscado', JSON.stringify(customer))
            }
          ...
          .then(async ({ html, customer, store, subject }) => {
            ...
            if (Number(storeId) === 1011) {
              console.log('Before send', JSON.stringify(customer), from)
            }
