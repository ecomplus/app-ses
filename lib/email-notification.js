...
logger.log('fui chamado')
const transactionalMails = require('@ecomplus/transactional-mails')
logger.log('templates', transactionalMails)
...
module.exports = ({ appSdk, configObj }) => {
  logger.log('import', JSON.stringify(configObj))
  return (trigger, storeId) => {
    ...
    if (storeId === 1011) {
      logger.log('Vamos comecar a processar o envio', JSON.stringify(body))
    }
    ...
    const queueAndSend = () => {
      ...
      if (storeId === 1011) {
        logger.log('Processar fila', JSON.stringify(processQueue))
      }
      setTimeout(async () => {
        let lastValidRecord
        Promise
          ...
          .then(([store, order]) => {
            if (storeId === 1011) {
              logger.log('Pegou loja', JSON.stringify(store), 'Pegou pedido', JSON.stringify(order))
            }
          ...
          .then(async ({ customer, order, store }) => {
            if (storeId === 1011) {
              logger.log('Cliente buscado', JSON.stringify(customer))
            }
            ...
            if (Array.isArray(order[subresource])) {
              ...
              if (storeId === 1011) {
                logger.log('Sorted records', JSON.stringify(sortedRecords))
              }
          ...
          .then(async ({ html, customer, store, subject }) => {
            ...
            if (storeId === 1011) {
              logger.log('Before send', JSON.stringify(customer), from)
            }
